...
GAME_PREFIXES = [
    ...
    "Chrono", "Secret of", "Tales of", "Dragon",
]
...
GAME_SUFFIXES = [
    ...
    "Championship", "Directors Cut", "Gold Edition",
]

# Console categories for organizing
CONSOLES = [
    "NES", "SNES", "N64", "GameCube", "Wii",
    "Genesis", "Dreamcast", "PlayStation", "PS2",
    "GameBoy", "GBA", "DS", "Arcade",
]

# File extensions for ROM files
ROM_EXTENSIONS = [
    ".zip", ".7z", ".nes", ".smc", ".sfc", ".z64", ".n64",
    ".iso", ".gba", ".nds", ".gen", ".md", ".bin", ".cue",
]
...
def generate_filename(include_region: bool = True, collision_target: str = None) -> str:
    ...
    if include_region and random.random() < 0.8:  # 80% have region tags
        # Add 1-3 region tags
        num_tags = random.choices([1, 2, 3], weights=[70, 25, 5])[0]
        tags = random.sample(REGION_TAGS, min(num_tags, len(REGION_TAGS)))
...
def create_collision_pair(base_name: str, ext: str) -> tuple[str, str]:
    ...
    """
    tag = random.choice(REGION_TAGS)
    file_with_tag = f"{base_name} ({tag}){ext}"
...
def generate_test_data(size: str) -> None:
    ...
    files_per_dir = regular_files // len(all_dirs)
    remainder = regular_files % len(all_dirs)
    ...
    print(f"  python3 /hoard/workspace/cleanfilenames-gui/cleanfilenames_core.py {TEST_DIR}")
    print(f"\nApply the changes with:")
    print(f"  python3 /hoard/workspace/cleanfilenames-gui/cleanfilenames_core.py {TEST_DIR} --apply")
